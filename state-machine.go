...
func (m *Machine) exec() {
	m.loop_sleep_sync()
	if _sh := m.bp.cfg.Start_handler; _sh != nil {
		_sh(m.bp.names[m.bp.last_state])
	}
	...
	select {
	case e := <-exec_chan:
		if _eh := m.bp.cfg.Err_handler; e != nil && _eh != nil {
			_eh(e)
			return
		}
...
func (m *Machine) next_step() {
	if m.bp.last_state+1 == len(m.bp.states) {
		m.bp.last_state = 0
		return
	}
	m.bp.last_state += 1
